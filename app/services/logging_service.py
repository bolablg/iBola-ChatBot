...
try:
    ...
    print("Google Cloud Logging not available. Install with: pip install google-cloud-logging")

from config import GCP_SA_CREDENTIALS_PATH, GCP_PROJECT_ID, LOG_LEVEL
...
class StructuredFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        extra_fields = ['session_id', 'user_language', 'agent_type', 'user_input', 'response_time']
...
class LoggingService:
    ...
    def __init__(self):
        ...
        if GOOGLE_CLOUD_AVAILABLE and os.path.exists(GCP_SA_CREDENTIALS_PATH):
            try:
                cloud_handler = GoogleCloudHandler(GCP_PROJECT_ID, GCP_SA_CREDENTIALS_PATH)
                if cloud_handler.logger:
                    ...
                    self.logger.warning("Google Cloud Logging handler failed to initialize")
            except Exception as e:
    ...
    def log_chat_interaction(self, session_id: str, user_input: str, agent_type: str,
                           response: str, response_time: float, user_language: str = 'en'):
