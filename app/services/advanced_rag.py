...
class HybridRetriever(BaseRetriever):
    ...
    def __init__(self, vectorstore_path: str, embedding_model: str = "models/embedding-001"):
        if not RAG_AVAILABLE:
            return
    ...
    def _build_term_index(self):
        ...
        try:
            ...
            for i, doc_content in enumerate(all_docs.get('documents', [])):
                doc_id = all_docs.get('ids', [])[i] if i < len(all_docs.get('ids', [])) else str(i)
    ...
    def _get_relevant_documents(self, query: str, *, run_manager=None) -> List[Document]:
        ...
        try:
                    ...
                    try:
                        ...
                        if doc.get('documents'):
                            keyword_docs.append(Document(
                                ...
                                metadata=doc.get('metadatas', [{}])[0] if doc.get('metadatas') else {}
                            ))
                    except Exception:
                        continue
            ...
            # 3. Reranking
            if len(all_docs) > 5:
                reranked_results = self.reranker.rerank_documents(query, all_docs, top_k=5)
...
class AdvancedRAGService:
    ...
    def rerank_documents(self, query: str, documents: List[Document], top_k: int = 5) -> List[Document]:
        ...
        reranked = self.hybrid_retriever.reranker.rerank_documents(query, documents, top_k)
    ...
    def get_retrieval_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "capabilities": ["query_expansion", "reranking", "hybrid_search", "keyword_search"]
        }
