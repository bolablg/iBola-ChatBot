...
class LanguageDetection:
    ...
    def _analyze_message_language(self, message: str) -> str:
        ...
        # French indicators
        if any(word in message_lower for word in ['le', 'la', 'les', 'et', 'est', 'je', 'tu', 'nous', 'vous']):
            return 'fr'

        # Spanish indicators
        if any(word in message_lower for word in ['el', 'la', 'los', 'las', 'y', 'es', 'soy', 'eres', 'somos']):
            return 'es'

        # German indicators
        if any(word in message_lower for word in ['der', 'die', 'das', 'und', 'ist', 'ich', 'du', 'wir', 'ihr']):
            return 'de'

        # Italian indicators
        if any(word in message_lower for word in ['il', 'lo', 'la', 'i', 'gli', 'le', 'e', 'è', 'sono']):
            return 'it'

        # Portuguese indicators
        if any(word in message_lower for word in ['o', 'a', 'os', 'as', 'e', 'é', 'sou', 'és', 'somos']):
            return 'pt'
    ...
    def get_agent_name(self, agent_type: str, language: str) -> str:
        ...
        lang_translations = self.agent_translations.get(language, self.agent_translations['en'])
